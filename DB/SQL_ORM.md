...
3. 나이가 30살 이상인 사람의 인원 수
   ...
   ```python
   # orm
   User.objects.filter(age__gt=30).count()
   ```
...
4. 나이가 20살 이하인 사람의 인원 수 
   ...
   ```python
   # orm
   User.objects.filter(age__lt=20).count()
   ```
...
7. 지역번호가 02인 사람의 인원 수
   ...
   ```python
   # orm
   User.objects.count().__startswith('02')
   ```
...
1. 전체 평균 나이
   ...
   ```python
   # orm
   User.objects.aggregate(average=Avg('age'))  # average라는 텍스트까지 같이 프린트
   User.objects.aggregate(Avg('age')) # 이렇게 표현하는게 정석
   ```
...
4. 계좌 잔액 중 가장 높은 값
   ...
   ```python
   # orm
   User.objects.order_by('-balance').values('balance')[0]
   ```
