...
> git은 분산버전관리시스템(Distributed Version Control System)
>
...
2. 초기 설치 완료 후 로컬 컴퓨터에 Author 정보를 설정해야한다.
   ...
   $ git config --global -l      // 설정된 내용을 확인할 수 있는 명령어
   ```
...
| ------------------------------------ | ------------------------------------------ | ---------------------------------------------- |
| 실제 작업되는 공간                   | commit 되기 전 임시로 파일들이 보여지는 곳 | git으로 관리되는 파일들의 버전들이 저장되는 곳 |
| 변경점이 나타나면 이곳에 파일이 등록 | 이곳에서 commit 되어도 되는지 파일을 확인  |                                                |

`git commit -a` : staging area에 자동으로 올려주고 commit을 시켜주는 친구 / 가급적이면 사용을 안하는 것이 좋다. 어떤 파일이 올라가는지 확인할 수 없기 때문.

`git commit-amend` : commit 메시지를 변경할 때 사용
...
- 프로젝트에 관련 없는 파일을 등록하여 commit 되지 않도록 하는 것
  ...
  - IDE 환경 설정 파일 (.idea/)
  - 가상환경 폴더 및 파일(venv/)
- `.gitignore`라는 파일을 생성 (확장자는 따로 없음)
  - 제외하고 싶은 파일을 등록
  - 파일명을 적어주면 끝
- gitignore.io를 이용하면 편하게 gitignore 파일을 작성할 수 있음
  - 단, 우리가 생성한 파일은 우리가 직접 등록해야 함(ex. 단순 참고용도인 파일)
...
- Working Directory 에서 Staging Area로 관리 파일들을 이동시키는 명령어
...
```bash
$ git commit -m "커밋 메시지를 잘 남기자! 유의미한 내용으로 작성"
```

- 버전 이력을 확정짓는 명령어
- 해당 시점의 파일 변경 상태를 스냅샷으로 기록해 남긴다.
...
​		`'git rm -rf --cached 폴더명'`으로 해당 폴더를 Staging Area에서 Working Directory로 내린다.
 	...
 	6. 그리고 다시 git status로 Staging Area에 올라온 상태를 파악하rh
 	7. git commit을 한다.
